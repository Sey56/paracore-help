...
- **Paracore MCP**: Connect Claude Desktop, Cursor, or any MCP client to Revit. The LLM writes C# using Paracore's fluent DSL, explores your model, and shows you the code before execution. One C# compiler replaces thousands of hardcoded AI tools.
...
### 2. Paracore MCP (AI-Powered Automation)
The **MCP server** lets any MCP-compatible AI client control Revit through natural language. It writes and executes C# on the fly using Paracore's fluent DSL. Every code change is shown for your approval before it touches the model.
