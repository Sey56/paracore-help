...
**Hydration** is the fastest way to get Revit data into your script. Instead of using strings or IDs, you use the actual Revit API types (e.g., `Level`, `WallType`, `Material`) as your property types.
...
The **Compute** (🔄) button is an intelligent tool that fetches live data from Revit.
...
> 💡 **Tip**: If you encounter an "ambiguous reference" error in VS Code for `[Required]`, you can use **`[Mandatory]`** instead. Both perform the same logical validation.



-   **Feedback**: Immediately to the right of the Run button, an **Execution Status** icon (✅/❌) will appear once the script finishes, allowing you to quickly check the result.
