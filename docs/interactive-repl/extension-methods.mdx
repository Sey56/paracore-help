...
- **Example**: `myWall.AllGeometry().Table();`
...
| Method | Description | Example |
| :--- | :--- | :--- |
| **`.Table()`** | Renders dynamic parameter tables for collections. | `walls.Table();` |
| **`.PieChart()`** / **`.BarChart()`** | Renders Pie/Bar charts in the Analytics tab. | `data.PieChart();` |
| **`.Select()`** / **`.Zoom()`** | Selects and focuses elements in the Revit viewport. | `room.Select();` |
| **`.Isolate()`** | Isolates elements in the active view. | `rooms.Isolate();` |
...
| **`Peek(el)`** | **Auto** | Forensic API vs UI value comparison. |
| **`ListParams(el)`** | **Auto** | Fast exhaustive parameter listings. |
| **`ListProperties(el)`** | **Auto** | Internal metadata (Level, Workset, Location). |
| **`ListGeometry(el)`** | **Auto** | Summary of geometry (Solid count, Volume, and Area). |
...
:::tip Pro Tip
If you need to filter or sort data *before* rendering it to a table, use the extension method equivalents (e.g., `el.AllParams().Table()`) instead of the auto-rendering global functions.
:::
