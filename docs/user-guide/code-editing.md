...
RAP provides a powerful integration with Visual Studio Code (VSCode) to offer a seamless and robust code editing experience for your C# scripts, complete with full IntelliSense and code completion for Revit API and .NET namespaces.
...
When you click "Edit in VSCode", RAP performs the following actions:

1.  **Temporary Workspace Generation:** RAP generates a temporary VSCode workspace. This workspace is scaffolded with necessary project files (`.csproj`, `.sln`) and a `Globals.cs` file, providing the context for IntelliSense and code completion for the Revit API and common .NET C# namespaces.
...
*   **Full IntelliSense:** Within VSCode, you will have full IntelliSense, code completion, and error checking for your C# scripts, including access to Revit API objects and custom RAP globals (like `Print`, `Transact`, `Show`).
...
*   **Real-time Reflection in RAP:** RAP continuously monitors the original script files. As you save changes in VSCode, RAP detects these updates, and the changes are reflected in real-time within the RAP application (e.g., in the floating code viewer, script cards, and Script Inspector).

## Re-opening an Existing Editing Session

If you wish to edit a script that you have previously opened in VSCode:

*   RAP will detect that a temporary workspace for that script already exists.
*   Instead of generating a new one, RAP will simply re-open the existing, scaffolded VSCode workspace, allowing you to continue your work seamlessly.

## Automatic Cleanup of Temporary Workspaces

To prevent cluttering your hard disk, RAP automatically manages the lifecycle of these temporary VSCode workspaces:
