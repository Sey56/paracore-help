...
# Git Workflow Basics in RAP

RAP integrates with Git to provide a powerful and flexible workflow for managing your automation scripts. Understanding basic Git concepts, especially branching, is crucial for effective collaboration.
...
## Typical Git Workflow for Developers and Admins in RAP
...
1.  **Create a New Branch:**
    ...
    *   Use the **"Create Branch" button** in RAP's `GitStatusPanel`.

2.  **Work and Commit in RAP:**
    *   Make your changes to the scripts within RAP.
    *   The `GitStatusPanel` will detect "uncommitted changes."
    *   Use the **"Commit" button** in RAP to save your changes to your *current feature branch*. This creates a snapshot of your work.

3.  **Push Your Branch:**
    *   Use the **"Push" button** in RAP to send your local feature branch (with its commits) to the remote Git repository.

4.  **Test in Revit:**
    *   While you're on your feature branch in RAP, any scripts you run in Revit will be the versions from *that branch*. This allows you to thoroughly test your new features or fixes in isolation.
...
7.  **Users Get the Updates:**
    *   Now that your changes are in `main`, when a `user` clicks their **"Update Scripts" button** in RAP, they will pull the latest changes from the `main` branch and get your new, tested, and approved script.

8.  **Developer Sync:** Developers should regularly use the **"Pull" button** in RAP to update their local `main` branch with the latest approved changes from the remote `main` branch.
